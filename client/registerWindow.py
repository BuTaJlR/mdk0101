...
from client.api.resolvers import register
...
class RegisterWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        lbl_main = tk.Label(self, text="Регистрация в системе", font=self.font)
        ...
        btn_enter = tk.Button(self, text='Создать аккаунт', font=self.font,
    ...
    def register(self):
        post = register(user_login=self.user_login.get(),
                        user_password=self.user_password.get())
        if post:
            tk.messagebox.showinfo(title="Login correct", message="Регистрация успешна")
            self.parent.loginComplete()
            self.destroy()
        else:
            tk.messagebox.showerror(title="Wrong login", message="ошибка")
